<template>
  <div
    class="job-info"
    v-if="isJobShow"
  >
    <div
      class="jump"
      @click="jump"
    >{{!this.schoolInfo || JSON.stringify(this.schoolInfo) === "{}"?'跳过':'取消'}}</div>
    <h3 class="title">填写报考职位信息</h3>
    <p class="tips">填写信息可获得更精准的模考报告</p>
    <div class="job-info-wrapper">
      <template v-for="(item, i) in form">
        <JobSelect
          :key="i"
          :value="item.title"
          :selectData="item.selectData"
          :placeholderStr="item.placeholderStr"
          v-if="item.title || item.selectData.length > 0"
          :isOpen="openIndex===i"
          @getNextSelect="getNextSelect($event, i)"
          @close="handleClose"
          @open="handleOpen(i)"
        ></JobSelect>
      </template>
    </div>
    <button
      class="submit-btn"
      :class="{'disabled-act': btnDisabled}"
      :disabled="btnDisabled"
      @click="handleFinish"
    >完成填写</button>
  </div>
</template>

<script>
  import api from '../../common/api'
  import JobSelect from './JobSelect'
  export default {
    components: { JobSelect },
    props: {
      isJobShow: {
        type: Boolean,
        default: false,
      },
      positionName: {
        type: String,
        default: '',
      },
      examType: {
        type: Number,
        default: 1,
      },
      schoolInfo: {
        type: Object,
        default: null,
      },
      matchId: {
        type: Number,
        default: null,
      },
      subjectId: {
        type: Number,
        default: 1,
      },
      paperId: {
        type: Number,
        default: 0,
      },
    },
    data() {
      return {
        btnDisabled: true,
        openIndex: -1,
        form: [
          {
            value: '', //机构性质
            title: '',
            placeholderStr: '请选择机构性质',
            pCode: 100,
            selectData: [],
            key: 'jgxz',
          },
          {
            value: '', //报考省份
            title: '',
            placeholderStr: '请选择报考省份',
            pCode: 1,
            selectData: [],
            key: 'bksf',
          },
          {
            value: '', //市州
            title: '',
            placeholderStr: '请选择报考市州',
            pCode: 10,
            selectData: [],
            key: 'bksz',
          },
          {
            value: '', // 区县
            title: '',
            placeholderStr: '请选择报考区县',
            pCode: 20,
            selectData: [],
            key: 'bkqx',
          },
          {
            value: '', //部门
            title: '',
            placeholderStr: '请选择报考部门',
            pCode: 30,
            selectData: [],
            key: 'bkbm',
          },
          {
            value: '', //司局
            title: '',
            placeholderStr: '请选择报考司局',
            pCode: 40,
            selectData: [],
            key: 'bksj',
          },
          {
            value: '', //报考职位
            title: '',
            placeholderStr: '请选择报考职位',
            pCode: 50,
            selectData: [],
            key: 'bkzw',
          },
          {
            value: '', //职位代码
            title: '',
            placeholderStr: '请选择职位代码',
            pCode: 60,
            selectData: [],
            key: 'zwdm',
          },
        ],
      }
    },

    computed: {
      name() {
        return this.form.map(item => item.title).join(',')
      },
    },
    mounted() {
      if (this.examType !== 1) {
        this.form.shift()
      }
      if (!this.schoolInfo || JSON.stringify(this.schoolInfo) === "{}") {
        this.getJobSelect(0, this.positionName).then(i => {
          if (this.form[i].selectData.find(item => item.selected)) {
            this.form[i].value = this.form[i].selectData.find(item => item.selected).value
            this.form[i].title = this.form[i].selectData.find(item => item.selected).title
          } else {
            return 
          }
          return this.getJobSelect(i + 1, this.name)
        })
      } else {
        this.btnDisabled = false
        for (const key in this.schoolInfo) {
          if (this.schoolInfo.hasOwnProperty(key)) {
            this.form.forEach(item => {
              if (item.key === key) {
                item.title = this.schoolInfo[key]
              }
            })
          }
        }
      }
    },

    methods: {
      async getJobContent(schoolId) {
        let ret
        try {
          if (this.subjectId == 14) {
            ret = await api.getJobContentSL({
              matchId: this.paperId,
              schoolId,
            })
          } else {
            ret = await api.getJobContent({
              matchId: this.matchId,
              schoolId,
            })
          }
          if (ret.data.code === 1000000) {
            for (const key in ret.data.data) {
              this.form.forEach(item => {
                if (item.key === key) {
                  item.title = ret.data.data[key].title
                  item.value = ret.data.data[key].value
                }
              })
            }
            this.form.forEach((item, i) => {
              if (item.value) {
                this.getJobSelect(i, this.name)
              }
            })
          }
        } catch (err) {
          console.log(err)
        }
      },
      handleClose() {
        this.openIndex = -1
      },
      handleOpen(i) {
        if (this.schoolInfo) {
          this.getJobSelect(i, this.name)
        }
        this.openIndex = i
      },
      getNextSelect({ title, value }, index) {
        this.openIndex = -1
        if (this.form[index].title && this.form[index].title !== title) {
          this.btnDisabled = true
          for (let i = index + 1; i < this.form.length; i++) {
            const item = this.form[i]
            item.value = ''
            item.title = ''
            item.selectData = []
          }
          this.btnDisabled = true
        }
        this.form[index].title = title
        this.form[index].value = value
        
        this.getJobSelect(index + 1, this.name)
      },
      jump() {
        this.$emit('update:isJobShow', false)
        document.getElementsByTagName('body')[0].style.overflow = ''
      },
      // 填写完成
      async handleFinish() {
        const schoolNameArr = []
        const body = {}
        this.form.forEach(item => {
          if (item.title) {
            schoolNameArr.push(item.title)
          }
          body[item.key] = item.title
        })

        const schoolName = schoolNameArr.join('-')
        let ret
        try {
          if (this.subjectId == 14) {
            ret = await api.jobApplySL({
              paperId: this.paperId,
              schoolName,
              body
            })
          } else {
            ret = await api.jobApply({
              matchId: this.matchId,
              schoolName,
              body
            })
          }
          console.log(ret)
          if (ret.data.code === 1000000) {
            this.Toast({
              // 申论和行测返回的message位置不统一
              message: ret.data.message || ret.data.data.message,
              duration: 2000,
              className: 'my_toast',
            })
            this.$emit('update:isJobShow', false)
            this.$emit('refresh')
            document.getElementsByTagName('body')[0].style.overflow = ''
          }
        } catch (err) {
          console.log(err)
        }
      },
      async getJobSelect(idx, name) {
        let i = idx
        console.log('idx', idx);
        console.log('this.form.length', this.form.length);
        if (idx === this.form.length) {
          this.btnDisabled = false
          return
        }
        try {
          let ret
          if (this.subjectId == 14) {
            ret = await api.getJobSelectSL({
              matchId: this.paperId,
              pCode: this.form[idx].pCode,
              name,
            })
          } else {
            ret = await api.getJobSelect({
              matchId: this.matchId,
              pCode: this.form[idx].pCode,
              name,
            })
          }

          if (ret.data.data.length === 0 && i <= this.form.length - 1) {
            i++
            if (this.form[idx].pCode === 60 && ret.data.length === 0) {
              this.btnDisabled = false
            }
            this.getJobSelect(i, this.name)
          } else {
            this.form[idx].selectData = ret.data.data
          }
          return i
        } catch (err) {
          console.log(err)
        }
      },
    },
  }
</script>
<style lang='less' scoped>
  .job-info {
    padding: 20px 40px 0 40px;
    overflow-y: auto;
    box-sizing: border-box;
    position: fixed;
    left: 0;
    top: 0;
    width: 100%;
    height: 100%;
    background: #fff;
    z-index: 111;
    .jump {
      height: 42px;
      font-size: 30px;
      font-family: PingFangSC-Regular, PingFang SC;
      font-weight: 400;
      color: #909090;
      line-height: 42px;
      text-align: right;
      margin-top: 10px;
    }
    .title {
      height: 50px;
      font-size: 36px;
      font-family: PingFangSC-Semibold, PingFang SC;
      font-weight: 600;
      color: #333333;
      text-align: center;
      line-height: 50px;
      margin-top: 24px;
    }
    .tips {
      height: 32px;
      font-size: 22px;
      font-family: PingFangSC-Regular, PingFang SC;
      font-weight: 400;
      color: #9b9b9b;
      line-height: 32px;
      text-align: center;
      margin-top: 12px;
      margin-bottom: 60px;
    }
    .job-info-wrapper {
      padding-bottom: 160px;
    }
    .submit-btn {
      width: 630px;
      height: 88px;
      transform: translateX(-50%);
      left: 50%;
      background: #ff6d73;
      border-radius: 44px;
      font-size: 36px;
      font-family: PingFangSC-Regular, PingFang SC;
      font-weight: 400;
      color: #ffffff;
      line-height: 88px;
      position: fixed;
      bottom: 60px;
      z-index: 13;
      &.disabled-act {
        color: #909090;
        background: #f3f3f3;
      }
    }
  }
</style>