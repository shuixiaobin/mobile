...
Util.formatTime = function(date) {
  ...
  var formatData02 =
    [year, month, day].map(Util.formatNumber).join('.') +
    ' ' +
    [hour, minute].map(Util.formatNumber).join(':')
  return { formatData, formatData02 }
}
...
Util.showThisMenuEle = (item, accessMap) => {
  if (item.meta && typeof item.meta.access === 'string') {
    if (item.meta.access in accessMap) {
      return true
    } else {
      return false
    }
...
Util.getMenuList = function getMenuListByRoutes(routes, accessMap) {
  ...
  routes.forEach(item => {
    ...
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      ...
      if (Util.showThisMenuEle(item, accessMap)) {
        res.push(obj)
      }
    }
  })
